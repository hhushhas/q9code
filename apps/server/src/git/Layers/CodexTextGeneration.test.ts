...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  ...
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "q9code-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__Q9CODE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__Q9CODE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "q9code-codex-text-" });
